...
usage: '<time in minutes> <reminder>',
run : async(client, message, args) => {
  let messagez = args.join(' ');
  if (messagez.length < 1) return message.channel.send('Incorrect format. >remind <time in minutes> <message>');
  return new Promise((resolve) => {
    ...
    if (results.length === 0) return message.channel.send('Error parsing date. Try using format: >reminder <minutes> <message>');
    ...
    setTimeout(() => {
     message.reply(`Reminder: "${messagez}"!`);
    }, minutes * 60000);
